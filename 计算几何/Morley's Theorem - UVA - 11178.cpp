...
struct Point
{
	...
	void out(void) { printf("%.6lf %.6lf", x, y); }
};
...
Point solver(Point A, Point B, Point C)
{
	Vector v1 = C - B;
	...
	return GetLineIntersection(Line{ B, v1 }, Line{ C, v2 });
}
int main(void)
{
	Point A, B, C;
	...
	while (T--)
	{
		...
		Point F = solver(A, B, C);
		Point E = solver(B, C, A);
		Point D = solver(C, A, B);
